...
// v18.0：去重从「SELECT 判重 → INSERT」改为 `submitReport()` 的单事务 + 评论行 FOR UPDATE，
//        原写法两句之间无锁无唯一键，20 并发实测落库 10 行（详见 lib/data.ts 的 submitReport 注释）。
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await submitReport(user.id, { type: "comment", commentId }, reason);
  if (!r.ok) {
    if (r.code === "not_found") {
